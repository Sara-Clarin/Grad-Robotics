...
class AdaRRT():
    ...
    def _get_random_sample(self):
        ...
        """
        # FILL in your code here
        return np.random.uniform(self.joint_lower_limits, self.joint_upper_limits, (len(self.joint_lower_limits)))
        
    def _get_random_sample_near_goal(self):
    	uniform_sample = self._get_random_sample()
    	biased_sample = np.random.uniform([l - 0.05 for l in self.goal.state], [u + 0.05 for u in self.goal.state], (len(self.goal.state)))
    	
    	return uniform_sample if np.random.uniform(0, 1, 1) > 0.2 else biased_sample
    ...
    def _extend_sample(self, sample, neighbor):
        ...
        new_node = neighbor.add_child(new_state) if not self._check_for_collision(new_state) else None
        return new_node
    ...
    def _check_for_completion(self, node):
        ...
        """
        # FILL in your code here
        distance_to_goal = np.linalg.norm(node.state - self.goal.state)
        
        if distance_to_goal <= self.goal_precision:
        	return True
