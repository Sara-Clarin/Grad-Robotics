...
def createBw():
    ...
    # Sample around the can at same height 
...
def main(if_sim):
    ...
    preshape = [1.0, 1.0] #needs to be between 0 and 1.6; assign a value the so that the fingers can encompass the can without gripping tightly
    ...
    # 1 Projection strategy iteration
    ...
    #q_error = J.T @ np.linalg.pinv(J @ J.T) @ delta_x
    #q = q - q_error
    #print(q)
    ...
    for _ in range(50):
    	q = arm_skeleton.get_positions()
    	print(q)
    	delta_x = np.array([0., 0., 0., -0.01, 0., 0.])
    	J = arm_skeleton.get_jacobian(hand.get_endeffector_body_node())
