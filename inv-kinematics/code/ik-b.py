...
def objective(x):
    ...
    sq_val = np.linalg.norm(fk(x, link_lengths) -  p_d)
    return np.dot(sq_val, sq_val.T)
...
def constraint1(x):
    ...
    """
    # FILL in your code here
    l_1 = np.array([ link_lengths[0] ])
    theta = np.array([ x[0] ])

    start =  np.array([0,0,0]) 
    end = fk( theta, l_1) 

    return -1 * (line_sphere_intersection(start, end, obstacle_center, obstacle_radius))
...
def constraint2(x):
    ...
    theta_prev = np.array([ x[0] ])
    l_1 = np.array( [link_lengths[0]] )
    start = fk( theta_prev, l_1 )
